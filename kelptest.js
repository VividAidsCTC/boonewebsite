...
setTimeout(() => {
        ...
        createSeafloorFeatures(); // Add rocks and features
    }, 1000);
...
function createSeafloorFeatures() {
    log('Creating seafloor features...');
    
    // Create rock clusters
    createRockClusters(25); // 25 rock clusters
    
    // Create coral formations  
    createCoralFormations(15); // 15 coral pieces
    
    // Create shells and debris
    createShellsAndDebris(40); // 40 small objects
    
    log('Seafloor features created');
}

function createRockClusters(count) {
    for (let i = 0; i < count; i++) {
        // Random position on seafloor (avoid kelp area)
        let x, z;
        do {
            x = (Math.random() - 0.5) * 800; // Spread across large area
            z = (Math.random() - 0.5) * 800;
        } while (Math.sqrt(x*x + z*z) < 20); // Keep away from kelp center
        
        // Create rock cluster (3-7 rocks per cluster)
        const rocksInCluster = 3 + Math.floor(Math.random() * 5);
        
        for (let j = 0; j < rocksInCluster; j++) {
            // Cluster rocks together
            const clusterX = x + (Math.random() - 0.5) * 8;
            const clusterZ = z + (Math.random() - 0.5) * 8;
            
            // Create rock geometry - irregular shapes
            const rockSize = 0.5 + Math.random() * 2.5;
            const rockGeometry = new THREE.SphereGeometry(rockSize, 8, 6);
            
            // Deform to make irregular
            const positions = rockGeometry.attributes.position;
            for (let k = 0; k < positions.count; k++) {
                const vertex = new THREE.Vector3();
                vertex.fromBufferAttribute(positions, k);
                vertex.multiplyScalar(0.7 + Math.random() * 0.6); // Random deformation
                positions.setXYZ(k, vertex.x, vertex.y, vertex.z);
            }
            rockGeometry.computeVertexNormals();
            
            // Rock material - varied grays and browns
            const grayVariation = 0.2 + Math.random() * 0.3;
            const brownTint = Math.random() * 0.1;
            const rockMaterial = new THREE.MeshPhongMaterial({
                color: new THREE.Color(grayVariation + brownTint, grayVariation, grayVariation),
                shininess: 1,
                specular: 0x111111
            });
            
            const rock = new THREE.Mesh(rockGeometry, rockMaterial);
            rock.position.set(clusterX, -1 + rockSize * 0.3, clusterZ); // Partially buried
...
function createCoralFormations(count) {
    for (let i = 0; i < count; i++) {
        // Random position away from kelp
        let x, z;
        do {
        ...
        // Create coral - branching cylinder structure
        const coralHeight = 1 + Math.random() * 3;
        const coralBase = new THREE.CylinderGeometry(0.3, 0.8, coralHeight, 6, 1);
        
        // Coral material - orange/pink/red variations
        const hue = 0.05 + Math.random() * 0.15; // Orange to red range
        ...
        const coral = new THREE.Mesh(coralBase, coralMaterial);
        coral.position.set(x, -1 + coralHeight/2, z);
        
        // Add some branches
        for (let j = 0; j < 2 + Math.floor(Math.random() * 3); j++) {
            const branchHeight = coralHeight * (0.3 + Math.random() * 0.4);
            const branch = new THREE.CylinderGeometry(0.1, 0.2, branchHeight, 4, 1);
            const branchMesh = new THREE.Mesh(branch, coralMaterial);
            
            branchMesh.position.set(
                (Math.random() - 0.5) * 0.6,
                coralHeight * 0.3 + Math.random() * coralHeight * 0.4,
                (Math.random() - 0.5) * 0.6
            );
            branchMesh.rotation.z = (Math.random() - 0.5) * Math.PI * 0.5;
            branchMesh.rotation.x = (Math.random() - 0.5) * Math.PI * 0.5;
...
function createShellsAndDebris(count) {
    for (let i = 0; i < count; i++) {
        // Random position across entire seafloor
        ...
        if (objectType === 0) {
            ...
            const shellGeometry = new THREE.SphereGeometry(0.2 + Math.random() * 0.3, 8, 6);
            shellGeometry.scale(1, 0.3, 1.2); // Flatten for shell shape
            
            const shellMaterial = new THREE.MeshPhongMaterial({
                color: new THREE.Color(0.9, 0.85, 0.75), // Cream/white
                shininess: 20
            });
            
            mesh = new THREE.Mesh(shellGeometry, shellMaterial);
            
        } else if (objectType === 1) {
            // Small rock/pebble
            const pebbleGeometry = new THREE.SphereGeometry(0.1 + Math.random() * 0.2, 6, 4);
            const pebbleMaterial = new THREE.MeshPhongMaterial({
                color: new THREE.Color(0.3 + Math.random() * 0.2, 0.25 + Math.random() * 0.2, 0.2 + Math.random() * 0.15),
                shininess: 2
            });
            
            mesh = new THREE.Mesh(pebbleGeometry, pebbleMaterial);
            
        } else {
            // Stick/driftwood
            const stickGeometry = new THREE.CylinderGeometry(0.05, 0.08, 1 + Math.random() * 2, 6, 1);
            const stickMaterial = new THREE.MeshPhongMaterial({
                color: new THREE.Color(0.25, 0.15, 0.1), // Dark brown
                shininess: 1
            });
            
            mesh = new THREE.Mesh(stickGeometry, stickMaterial);
            mesh.rotation.z = Math.random() * Math.PI; // Random orientation
        }
        
        mesh.position.set(x, -0.95, z); // Slightly above seafloor
